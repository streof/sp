...
impl<'a, R: BufRead> StartsWith for Searcher<'a, R> {
    fn no_line_number(&mut self) -> SearcherResult {
        ...
        reader.for_byte_line_with_terminator(|line| {
            if line.contains_str(pattern) {
                sir.check_and_store_nln(pattern, line, check_starts_with);
            }
            Ok(true)
        })?;
        ...
        reader.for_byte_line_with_terminator(|line| {
            if line.is_ascii() {
                ...
                if line_lower.contains_str(pattern) {
                    sir.check_and_store_separate_nln(pattern, &line_lower, line, check_starts_with);
                }
            } else {
                let mut buf = Default::default();
                line.to_lowercase_into(&mut buf);
                if buf.contains_str(pattern) {
                    sir.check_and_store_separate_nln(pattern, &buf, line, check_starts_with);
                }
            }
            Ok(true)
        })?;
    ...
    fn no_line_number_max_count(&mut self) -> SearcherResult {
        ...
        reader.for_byte_line_with_terminator(|line| {
            ...
            if line.contains_str(pattern) {
                sir.check_and_store_nln_max_count(
                    pattern,
                    line,
                    &mut matches_left,
                    check_starts_with,
